...
class Inferencer():
    ...
    def inference_example_batch(self, premise: list, hypo: list, op1: str = "max", op2: str = "max"):
        """
        inference a example,
        premise: list
        hypo: list
        ...
        for one_pre, one_hypo in tqdm(zip(premise, hypo), desc="Evaluating", total=len(premise), disable=(not self.verbose)):
            out_score.append(self.inference_per_example(one_pre, one_hypo, op1, op2))
        
        return None, torch.tensor(out_score), None

    def inference_per_example(self, premise:str, hypo: str, op1: str = "max", op2: str = "max"):
        """
        inference a example,
        premise: string
        hypo: string
        ...
        if self.nlg_eval_mode is not None:
            if self.nlg_eval_mode == 'nli_sp':
                ...
                output_score = self.inference(premise_sent_mat, hypo_sents_mat)[2][:,:] ### use NLI head OR ALIGN head
                output_score_e = output_score[:,0] # entailment
                output_score_n = output_score[:,1] # neutrality
                output_score_c = output_score[:,2] # contradiction
                if op1 == "max":
                    output_score_e = output_score_e.view(len(premise_sents), len(hypo_sents)).max(dim=0)
                    output_score_c = output_score_c.view(len(premise_sents), len(hypo_sents)).max(dim=0)
                    output_score_n = output_score_n.view(len(premise_sents), len(hypo_sents)).max(dim=0)
                elif op1 == "min":
                    output_score_e = output_score_e.view(len(premise_sents), len(hypo_sents)).min(dim=0)
                    output_score_c = output_score_c.view(len(premise_sents), len(hypo_sents)).min(dim=0)
                    output_score_n = output_score_n.view(len(premise_sents), len(hypo_sents)).min(dim=0)
                elif op1 == "mean":
                    output_score_e = output_score_e.view(len(premise_sents), len(hypo_sents)).mean(dim=0)
                    output_score_c = output_score_c.view(len(premise_sents), len(hypo_sents)).mean(dim=0)
                    output_score_n = output_score_n.view(len(premise_sents), len(hypo_sents)).mean(dim=0)

                if op2 == "max":
                    output_score_e = output_score_e.values.max().item()
                    output_score_c = output_score_c.values.max().item()
                    output_score_n = output_score_n.values.mean().item()
                elif op2 == "min":
                    output_score_e = np.min(output_score_e.numpy()) # output_score_e.values.min().item()
                    output_score_c = np.min(output_score_c.numpy()) # output_score_c.values.min().item()
                    output_score_n = np.min(output_score_n.numpy()) # output_score_n.values.mean().item()
                elif op2 == "mean":
                    output_score_e = np.mean(output_score_e.numpy()) # output_score_e.values.mean().item()
                    output_score_c = np.mean(output_score_c.numpy()) # output_score_c.values.mean().item()
                    output_score_n = np.mean(output_score_n.numpy()) # output_score_n.values.mean().item()


                # output_score_e = output_score_e.view(len(premise_sents), len(hypo_sents)).max(dim=0).values.max().item() ### sum or mean depends on the task/aspect
                # output_score_c = output_score_c.view(len(premise_sents), len(hypo_sents)).max(dim=0).values.max().item() ### sum or mean depends on the task/aspect
                # output_score_n = output_score_n.view(len(premise_sents), len(hypo_sents)).max(dim=0).values.mean().item() ### sum or mean depends on the task/aspect

                return [output_score_e, output_score_c, output_score_n]
    ...
    def inference(self, premise, hypo):
        ...
        if self.nlg_eval_mode is not None:
            if self.nlg_eval_mode == 'nli':
                output_score_nli_e = output_score_tri[:,0] # extract 1st col from first row
                output_score_nli_n = output_score_tri[:,1] # extract 2nd col from first row
                output_score_nli_c = output_score_tri[:,2] # extract 3rd col from first row
                output_score_nli = torch.cat([output_score_nli_e, output_score_nli_c, output_score_nli_n])
    ...
    def nlg_eval(self, premise, hypo, op1: str = "max", op2: str = "max"):
        ...
        if (self.nlg_eval_mode == 'bin') or (self.nlg_eval_mode == 'nli') or (self.nlg_eval_mode == 'reg'):
            ...
            return self.inference_example_batch(premise, hypo, op1, op2)
